...
public class Assignment implements EweduElement {
  ...
  Assignment(String type, String unixTime, String title, String body)
      throws TitleTooLongException, BodyTooLongException, IllegalArgumentException {
    this(type, title);
  ...
  Text getCompletableTitle(int id) {
    return Text.builder(title, title.toPlain())
        .onHover(TextActions.showText(Text.of(TextColors.LIGHT_PURPLE,
            "Click here to toggle completion of this assignment")))
  ...
  private void setTitle(Text title) throws TitleTooLongException {
    if (title.toPlain().length() > MAXIMUM_TITLE_LENGTH) {
  ...
  private void setBody(Text body) throws BodyTooLongException {
    if (body.toPlain().length() > MAXIMUM_BODY_LENGTH) {
  ...
  /**
   * Formats this Assignment into a readable format to be sent in-game or to the console.
   * @param id The id of this Assignment
   * @return The Text containing all vital information about this Assignment object
   */
  ...
  Text formatReadableVerbose(int id) {
    return Text.builder(id + ". ").color(TextColors.RED)
        ...
        .append(Text.of(
            ...
            "\nCompleted By: ",
            TextColors.AQUA,
            String.join(", ", getPlayersCompletedNames())))
        .build();
  }
