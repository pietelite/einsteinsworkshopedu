...
public class AssignmentCommand extends EinsteinsWorkshopCommand {
	...
	public void onList(CommandSource source) {
		...
		if (plugin.getFeatures().get(EweduPlugin.FeatureTitle.ASSIGNMENTS).getManager().getElements().isEmpty()) {
	...
	public void onAdd(CommandSource source, String type, String[] title) {
		...
		try {
			...
			((AssignmentManager) plugin.getFeatures().get(EweduPlugin.FeatureTitle.ASSIGNMENTS).getManager()).getElements().add(new Assignment(type, String.join(" ", title)));
	...
	public void onInfo(CommandSource source, int id) {
		...
		try {
			source.sendMessage(((Assignment) plugin.getFeatures().get(EweduPlugin.FeatureTitle.ASSIGNMENTS).getManager().getElements().get(id - 1)).formatReadableVerbose(id));
		} catch (IndexOutOfBoundsException e) {
			source.sendMessage(Text.of(TextColors.RED, "There is no assignment with that id."));
		}
	}
	...
	public void onRemove(CommandSource source, int id) {
		...
		if (plugin.getFeatures().get(EweduPlugin.FeatureTitle.ASSIGNMENTS).getManager().getElements().remove(id - 1) != null) {
